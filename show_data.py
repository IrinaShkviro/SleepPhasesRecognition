...
datapath = 'D:/Irka/Projects/NeuralNetwok/data/data/'
...
def get_userlist():
	return ['002','003','005','007','08a','08b','09a','09b',
...
def show_distribution(patient):
    print patient
    pdta = np.load("%sp%s.npy"%(datapath,patient)).view(np.recarray)
    distribution = np.zeros((7), dtype='int64')
    pdta.gt[np.where(pdta.gt==7)] = 4
    for label in pdta.gt:
...
if __name__ == "__main__":
    for patient in get_userlist():
        show_distribution(patient)
