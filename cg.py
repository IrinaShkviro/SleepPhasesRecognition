...
def finetune_functions_sda_cg(sda, datasets, window_size):
    ...
    x = sda.input  # data, presented as window with x, y, x for each sample
    y = T.iscalar('y')  # labels, presented as int label

    validate_model = theano.function(
        [index],
        outputs=sda.logLayer.errors(y),
        givens={
            x: valid_set_x[index: index + window_size],
            y: valid_set_y[index + window_size - 1]
        },
    ...
    finetune_cost = sda.logLayer.negative_log_likelihood(y)
    finetune_error = sda.logLayer.errors(y)
